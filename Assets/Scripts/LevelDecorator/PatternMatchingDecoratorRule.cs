...
public class PatternMatchingDecoratorRule : BaseDecoratorRule
{
    ...
    [SerializeField] private Array2DWrapper<TileType> _placement;
    [SerializeField] private Array2DWrapper<TileType> _fill;
    internal override bool CanBeApplied(TileType[,] levelDecorated, Room room)
    {
        if (FindOccurrences(levelDecorated, room).Length > 0) { return true; }
        return false;
    }
    ...
    private Vector2Int[] FindOccurrences(TileType[,] levelDecorated, Room room)
    {
        ...
        for (int y = room.Area.position.y - 1; y < room.Area.position.y + room.Area.height + 2 - _placement.Height; y++)
        {
            for (int x = room.Area.position.x - 1; x < room.Area.position.x + room.Area.width + 2 - _placement.Width; x++)
            {
                if (IsPatternAtPosition(levelDecorated, _placement, x, y))
                {
    ...
    private bool IsPatternAtPosition(TileType[,] levelDecorated, Array2DWrapper<TileType> pattern, int startX, int startY)
    {
        for (int y = 0; y < pattern.Height; y++)
        {
            ...
            {
                if (levelDecorated[startX + x, startY + y] != pattern[x, y]) { return false; }
            }
        }
        return true;
    }
